...
/*
 * Post-sourcing function If a user has to change the item on a PO, the
 * previously committed values are captured and filled back in after the new
 * item post sourcing event.
 */
function postSource_restoreLine(type, name) {
	...
	if (type == 'item' && name == 'item') {
		...
		try {
			...
			var stDescription = nlapiGetLineItemValue(type, 'description',
					lineNum);
			var curRate = nlapiGetLineItemValue(type, 'rate', lineNum);
			var departmentId = nlapiGetLineItemValue(type, 'department',
					lineNum);
			var classId = nlapiGetLineItemValue(type, 'class', lineNum);
			var customerId = nlapiGetLineItemValue(type, 'customer', lineNum);
			var capX = nlapiGetLineItemValue(type, 'custcol_capex_spec',
					lineNum);
			var isBillable = nlapiGetLineItemValue(type, 'isbillable', lineNum);
			var dteExRcpt = nlapiGetLineItemValue(type, 'expectedreceiptdate',
					lineNum);
			...
			if (stDescription != '' && stDescription != null) {
				nlapiSetCurrentLineItemValue(type, 'description',
						stDescription, false);
			}
			...
			if (departmentId != null && departmentId != '') {
				nlapiSetCurrentLineItemValue(type, 'department', departmentId,
						false);
			}
			...
			if (customerId != null && customerId != '') {
				nlapiSetCurrentLineItemValue(type, 'customer', customerId,
						false);
			}
			if (isBillable == 'T') {
				nlapiSetCurrentLineItemValue(type, 'isbillable', isBillable,
						false);
			}
			if (dteExRcpt != null && dteExRcpt != '') {
				nlapiSetCurrentLineItemValue(type, 'expectedreceiptdate',
						dteExRcpt, false);
			}
			if (capX != null && capX != '') {
				nlapiSetCurrentLineItemValue(type, 'custcol_capex_spec',
						isCapX, false);
			}

		} catch (e) {
			nlapiLogExecution('ERROR',
					'unexpected error restoring previous line values', e
							.toString());
		}
	}
}
...
/*
 * Validate Line CapX. if the line is > 3,000, the capx field is checked and if
 * there is no CAPEX SPEC value, the user is alerted to select one before
 * continuing.
 */
function validateLine_checkCapx(type) {
	if (type == 'item' || type == 'expense') {
		...
		/*
		 * Felix: this is your script. i shortened it and adjusted it to make it
		 * functional with both expenses and items
		 */
		if (customerId != null && customerId != '') {
...
/*
 * If any line is over 3,000 Dollars, this function ensures that the capex box
 * is checked and alerts users if they need to select a CAPEX SPEC before
 * saving.
 */
function save_checkCapX() {
	...
	for (var i = 1; i <= lineCount; i++) {
		...
		if (curAmount >= 3000) {
			...
			// if the CAPEX SPEC field is empty then alert the user and falsify
			// the save
			if (capXline == null || capXline == '') {
				alert('Line: ' + stItem
						+ ' requires CAPEX SPEC to be filled in.' + '\n'
